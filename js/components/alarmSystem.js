components.alarmSystem = {
    ...
    template:/*html*/ `
     <div class="ui col mini-1 bg-no h-xl-290 h-mini-650">
        <div class="col-content overflow">
            ...
            <div class="col-110 align-middle-center dev-statusChange glow pad-no hidden-phone" >
            <button :class="[colorMute(alarm.Status)]" :style="[statusWarning(alarm)]" v-on:click="statusModify(index, alarm.Status, 2)" :id="'mute_' + index">Mute</button></div>
            </li>
            </ul>
        </div>
     </div>
  ...
  orderedAlarms() {
    ...
    colorStatus(type, trigger){
        ...
        statusWarning(alarm) {
  if (!alarm || typeof alarm.alarmType === 'undefined') return '';
  return alarm.alarmType === 1
    ? this.warningMute.warningOn
    : this.warningMute.warningOff;
}
,
        getAlarmName(id) {
